/*
Word scrambler
Create a function that returns a value of a word and scrambles the
letter order with Math.random():
...
3. Just like an array, strings also have a length by default.
You can use this length to set the loop maximum value.
...
4. Create an empty temporary string variable that you can use to...
hold the new scrambled word value.
...
function scrambler(wordParam) {
    for (let index = wordParam.length; index > 0; --index) {
        toBeTaken = Math.floor(Math.random() * (wordParam.length - 0) + 0);
        scrambledWord += wordParam[toBeTaken];
        wordParam = wordParam.slice(0, toBeTaken) + wordParam.slice(toBeTaken +1);
        console.log("old word =" + wordParam);
